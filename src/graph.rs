...
impl<'a> Clone for Node<'a> {
    fn clone(&self) -> Self {
        match self {
            Node::VariableNode { value, source } => Node::VariableNode { value: value.clone() , source: None },
            Node::UnaryNode { op, value } => Node::VariableNode { value: Variable::new(0.0) , source: None },
            Node::BinaryNode { op, lhs, rhs } => Node::VariableNode { value: Variable::new(0.0) , source: None }
        }
    }
}
            ...
            Node::BinaryNode { op, lhs, rhs } => {
                ...
                match op {
                    ...
                    BinaryOp::Pow => {
                        update_grads(
                            &mut rhs_grads,
                            lhs_value.value.ln() * lhs_value.value.powf(rhs_value.value),
                        );
                        update_grads(
                            &mut lhs_grads,
                            rhs_value.value * lhs_value.value.powf(rhs_value.value - 1.0),
                        );
